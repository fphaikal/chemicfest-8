      ...
      <div class="flex flex-col md:flex-row gap-3">
        <div class="w-full md:w-2/3 flex flex-col gap-3">
          ...
          <div class="p-5 bg-gray-50 dark:bg-dark-1 rounded-xl">
            <p class="text-sm text-dark dark:text-white">Chemicalistronic Festival (CHEMICFEST) #8 Merupakan Festival tahunan sebagai sarana hiburan terutama
              untuk Warga SMK-SMTI Yogyakarta dari padatnya rutinitas pembelajaran. Kegiatan ini sebagai wadah untuk
              menampilkan bakat-bakat dari siswa-siswi SMK-SMTI Yogyakarta. Chemicfest juga menjadi ajang bagi siswa-siswi
              yang mengikuti ekstrakurikuler untuk memamerkan kreativitas yang mereka miliki. Beberapa Guest Star juga turut
              diundang guna memeriahkan CHEMICFEST #8.</p>
          </div>
        </div>
        <div class="w-full md:w-1/3 flex flex-col">
          <div class="flex flex-row">
            <div class="bg-gray-50 dark:bg-dark-1  w-full h-fit shadow-3xl rounded-2xl">
              ...
              <div  class="p-3 border-t dark:border-zinc-700">
                <label class="input flex items-center gap-2 bg-white dark:bg-dark">
                  <input v-model="comment" @keyup.enter="postComment" name="comment" type="text" class="grow bg-white dark:bg-dark text-dark dark:text-white" placeholder="Kirim Pesan" maxlength="200" required/>
...
export default {
  ...
  methods: {
    ...
    async getStream() {
      try {
        this.stream = await getAll('get/storage/json/stream');
      } catch (error) {
        console.error(error);
      }
    },
